...
/**
 *
 * This class finds the number of public variables in the class..
 *
 * @author Jaidev Kochunni (with some source code from 
 * LineCountSourceAnalyzer by Andrew Tasso
 * @version %I% %G%
 */
public class PublicVarCountSourceAnalyzer extends SourceAnalyzer
{
   ...
   */
  public PublicVarCountSourceAnalyzer()
  {
    this.resetAnalysis();
  }
   ...
   * then find the key word public in there and we've got the private variable for the class.
   */
  ...
  public List<String> generateConsoleReport()
  {
    ...
    consoleReport.add("Number of Public Variables:");
    consoleReport.add("");
  ...
  public List<List<String>> generateWorksheetReport()
  {
    ...
    for (String currClassKey : classKeySet)
    {
      ...
      Integer publicVars = (Integer) classLinesMap.get(currClassKey);
      
      currRow.add(currClassKey);
      currRow.add(publicVars + "");
      
      totalVars += publicVars;
